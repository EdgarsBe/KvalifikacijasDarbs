...
<div id="Dashboard" class="h-full">
    ...
    <div class="h-[95%] m-8 flex flex-row">
        <div class="flex flex-col mx-4 w-1/2">
            <form action="{{ route('ChangePFP') }}" method="post" enctype="multipart/form-data" class="flex items-center bg-primary-50 border 1 rounded border-primary-200 h-1/5 my-4">
                @csrf
                @method('put')
                <label class="bg-primary-200 m-6 h-24 w-24 cursor-pointer rounded-full border 1 border-primary-300 overflow-hidden object-contain"  id="fileUpload">
                    <img class="h-full object-cover" src="{{ $users->getImgURL() }}">
                </label>
                <input type="file" id="fileInput" name="pfp" class="hidden" >
                <button>Save Profile Picture</button>
            </form>
            <div class="bg-primary-50 border 1 rounded border-primary-200 h-4/5 my-4">
                <div class="m-4">
                    <h2>Addresses</h2>
                </div>
            </div>
        ...
        <div class="flex flex-col mx-4 w-1/2">
            <div class="bg-primary-50 border 1 rounded border-primary-200 h-1/2 my-4">
                <div class="m-4">
                    <h2 class="text-xl">Personal Information</h2>
                    <form method="post" action="{{ route('ChangeName') }}" class="my-4 flex flex-col justify-center">
                        @csrf
                        <h3>Firstname</h3>
                        <input name="firstname" class="w-full p-2 my-2 outline-none border-primary-200 border-2 rounded-lg bg-primary-100" value="{{ $users->firstname }}" placeholder="FirstName" required>
                        <h3>Lastname</h3>
                        <input name="lastname" class="w-full p-2 my-2 outline-none border-primary-200 border-2 rounded-lg bg-primary-100" value="{{ $users->lastname }}" placeholder="LastName" required>
                        <h3>Username</h3>
                        <input name="name" class="w-full p-2 my-2 outline-none border-primary-200 border-2 rounded-lg bg-primary-100" value="{{ $users->name }}" placeholder="Username" required>
                        <button type="submit" class="transition duration-500 text-black w-1/3 py-2 rounded-xl bg-primary-300 hover:bg-primary-200 active:bg-primary-400">Save Changes</button>
                    </form>
                </div>
            </div>
            <div class="bg-primary-50 border 1 rounded border-primary-200 h-1/2 my-4">
                <div class="m-2">
                    <div class="m-4 my-8">
                        <div class="m-4 flex gap-4"> 
                            ...
                            <button class="transition duration-500 text-black w-24 py-2 rounded-xl bg-primary-300 hover:bg-primary-200 active:bg-primary-400">Change</button>
                            <p>Not Working Yet</p>
                        </div>
                        <div class="m-4 flex gap-4">
                            <h3 class="my-2">Change Password</h3>
                            <button id="UpdateBtn" class="transition duration-500 text-black w-24 py-2 rounded-xl bg-primary-300 hover:bg-primary-200 active:bg-primary-400">Change</button>
                        </div>
                        <div class="m-4 flex gap-4">
                            <h3 class="my-2">Delete Account</h3>
                            <button id="DeleteBtn" class="transition duration-500 text-black w-24 py-2 rounded-xl bg-primary-300 hover:bg-primary-200 active:bg-primary-400">Delete</button>
                        </div>
                        @if(session('success'))
                            <p>{{ session('success') }}</p>
                        @endif
...
<div id="PasswordForm" class="h-full w-full flex justify-center items-center hidden">
